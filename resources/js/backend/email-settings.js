function initEmailSettings() {
    ...
    if (!emailForm || !emailBtn) return;

    emailForm.addEventListener('submit', async function (e) {
        e.preventDefault();
        clearValidationErrors(emailForm);
        ...
        try {
            ...
            if (response.status === 422) {
                showValidationErrors(emailForm, data.errors);
            ...
            if (response.ok && data.status) {
                if (typeof toastr !== 'undefined') {
                    toastr.success(data.message || 'Email settings saved successfully.');
                } else {
            ...
            console.error(err);
            if (typeof toastr !== 'undefined') {
                toastr.error('An error occurred while saving email settings.');
            } else {
